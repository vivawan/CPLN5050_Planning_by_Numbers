...
not_in_station <- as.data.frame(setdiff(name_rider, name_stop))#station in ridership but not in stop shapefile
...
### 2017 Bike Stops
sum(unique(bike_rider_17_start$bike_stop) %in% unique(bike_rider_17_end$bike_stop))#199
bike_stop_17 <- bike_rider_17 %>%
  ...
  filter(Lat != 0 & Long != 0) %>%
  rename(bike_stop = Name)#196 (3 stations have no coordinates)
...
## correlation plot
```{r dot cor, warning=FALSE, message=FALSE}
##2 017 acs dot
# Select only numeric variables and remove rows with missing values
numericVars <- final_17_total_acs.dot %>%
  select_if(is.numeric) %>%  # Select only numeric variables
  na.omit()  # Remove rows with missing values

## built environment
numericVars1<-numericVars%>%
  select(total_rider_tran, total_rider_bike,total_docks,bus_stop,pop.tot,pop.den,
         commuter.tot,hh.tot,hh.den, job, job.den)
ggpairs(numericVars1)
#-> 

## socialecnomic
numericVars2<-numericVars%>%
  select(total_rider_tran, total_rider_bike,bus_stop,pop.den,
         commuter.tot,job.den, white.share, veh.ownership, medInc,pover.rate,labor.rate,employ.rate,tran.share)
ggpairs(numericVars2)
#-> 

## results of initial variable selection
numericVars3<-numericVars%>%
  select(total_rider_tran, total_rider_bike,bus_stop,pop.den,
         commuter.tot,job.den, white.share, veh.ownership, medInc, employ.rate,tran.share
         )
ggpairs(numericVars3)

## add log
numericVars4 <- numericVars3%>%
  mutate(log_rider_tran = log(total_rider_tran+1),
         log_rider_bike = log(total_rider_bike+1),
         log_job_den = log(job.den+1))%>%
  select(-total_rider_tran, -total_rider_bike, -job.den)
ggpairs(numericVars4)

```

```{r buffer cor, warning=FALSE, message=FALSE}
## 2017 acs dot
# Select only numeric variables and remove rows with missing values
numericVars.buf <- final_17_total_acs.buffer %>%
  select_if(is.numeric) %>%  # Select only numeric variables
  na.omit()  # Remove rows with missing values

## built environment
numericVars.buf1<-numericVars%>%
  select(total_rider_tran, total_rider_bike,total_docks,bus_stop,pop.all,ave.pop.den,
         commuter.all,hh.all,ave.hh.den, job.all, ave.job.den)
ggpairs(numericVars.buf1)
#-> 

## socialecnomic
numericVars.buf2<-numericVars%>%
  select(total_rider_tran, total_rider_bike,bus_stop,ave.pop.den,
         commuter.all,ave.job.den, ave.white.share, ave.veh.ownership,
         ave.pover.rate,ave.labor.rate,ave.employ.rate,ave.tran.share)
ggpairs(numericVars.buf2)
#-> 
...
rm(list = setdiff(ls(), c("tran_line", "tran_stop", "tran_rider_17_period", "tran_rider_17_total",
                          ...
                          "acs.2017.stop.dot","acs.2017.stop.buffer",
                          "acs.2019.stop.dot","acs.2019.stop.buffer")))

## 2017 without Time Period
final_17_total_bike <- tran_rider_17_total %>%
  st_buffer(radius) %>%
...
final_17_total_bus <- tran_rider_17_total %>%
  st_buffer(radius) %>%
  st_join(bus_stop) %>%
  na.omit() %>%
  group_by(tran_stop) %>%
...
final_17_total_acs.dot <- final_17_total_bike %>%
  left_join(., final_17_total_bus, by = c("tran_stop")) %>%
  rename(bus_stop = n) %>%
  ...
  left_join(., acs.2017.stop.dot, by = c("tran_stop")) %>%
  na.omit() %>%
  select(-geometry)

final_17_total_acs.buffer <- final_17_total_bike %>%
  left_join(., final_17_total_bus, by = c("tran_stop")) %>%
  rename(bus_stop = n) %>%
  mutate(bus_stop = replace_na(bus_stop, 0)) %>%
  left_join(.,tran_rider_17_total %>%
              select(tran_stop, total_rider_tran, terminus), by = c("tran_stop")) %>%
  left_join(., acs.2017.stop.buffer, by = c("tran_stop")) %>%
  na.omit() %>%
  select(-geometry)
...
final_19_total_acs.dot <- final_19_total_bike %>%
  left_join(., final_19_total_bus, by = c("tran_stop")) %>%
  rename(bus_stop = n) %>%
  mutate(bus_stop = replace_na(bus_stop, 0)) %>%
  left_join(.,tran_rider_19_total %>%
              select(tran_stop, total_rider_tran, terminus), by = c("tran_stop")) %>%
  left_join(., acs.2019.stop.dot, by = c("tran_stop")) %>%
  na.omit() %>%
  select(-geometry)

final_19_total_acs.buffer <- final_19_total_bike %>%
  left_join(., final_19_total_bus, by = c("tran_stop")) %>%
  rename(bus_stop = n) %>%
  mutate(bus_stop = replace_na(bus_stop, 0)) %>%
  left_join(.,tran_rider_19_total %>%
              select(tran_stop, total_rider_tran, terminus), by = c("tran_stop")) %>%
  left_join(., acs.2019.stop.buffer, by = c("tran_stop")) %>%
  na.omit() %>%
  select(-geometry)
...
```{r step1}
rm(list = setdiff(ls(), c("final_17_total_acs.dot", "final_17_total_acs.buffer",
                          "final_19_total_acs.dot", "final_19_total_acs.buffer")))

## 2017
moda.0 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1), data = final_17_total_acs.dot)
summary(moda.0)

## 2019
modb.0 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1), data = final_19_total_acs.dot)
summary(modb.0)
```

```{r acs buffer}

modb.0 <- lm(total_rider_tran ~ total_rider_bike, data = final_19_total_acs.dot)
summary(modb.0)
```
...
total_rider_bike, bus_stop, pop.den, commuter.tot, job.den, white.share, veh.ownership, medInc, employ.rate, tran.share

```{r step2}
hist(final_17_total_acs.dot$)
table(final_17_total_acs.dot$)
summary(final_17_total_acs.dot)
## 2017
moda.1 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + log(pop.den + 1) + log(commuter.tot + 1) + log(job.den + 1) + white.share + log(veh.ownership + 1) + log(medInc + 1) + log(employ.rate + 1) + log(tran.share + 1), data = final_17_total_acs.dot)
summary(moda.1)
vif(moda.1)

moda.2 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + log(ave.pop.den + 1) + log(commuter.all + 1) + log(ave.job.den + 1) + ave.white.share + log(ave.veh.ownership + 1) + log(ave.employ.rate + 1) + log(ave.tran.share + 1), data = final_17_total_acs.buffer)
summary(moda.2)
vif(moda.2)

## 2019

modb.1 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + log(pop.den + 1) + log(commuter.tot + 1) + log(job.den + 1) + white.share + log(veh.ownership + 1) + log(medInc + 1) + log(employ.rate + 1) + log(tran.share + 1), data = final_19_total_acs.dot)
summary(modb.1)
vif(modb.1)

modb.2 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop, data = final_19_total_acs.buffer)
summary(modb.2)
vif(modb.2)
```

```{r}
test1 <- final_17_total_acs.dot %>%
  filter(total_rider_bike != 0)

test2 <- final_17_total_acs.dot %>%
  filter(total_rider_bike == 0)

mod1 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + pop.den + commuter.tot + job.den + white.share + veh.ownership + medInc + employ.rate + tran.share, data = test1)
summary(mod1)
vif(mod1)

mod2 <- lm(log(total_rider_tran) ~ bus_stop + pop.den + veh.ownership + tran.share, data = test2)
summary(mod2)
vif(mod2)

mod3 <- lm(log(total_rider_tran) ~ bus_stop + log(pop.den + 1) + log(commuter.tot + 1) + log(job.den + 1) + white.share + log(veh.ownership + 1) + log(medInc + 1) + log(employ.rate + 1) + log(tran.share + 1), data = test2)
summary(mod3)
vif(mod3)
