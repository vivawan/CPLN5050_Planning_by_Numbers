...
dat.logit <- mlogit.data(dat, shape="wide", 
                      choice="mode_cat", 
                      varying=c(22:27)) #specify the variable vary by mode
...
## what it did: transit.time / car.time -> time = column, transit/car -> values/alternatives
...
dat.1.logit <- mlogit.data(dat.1, shape="wide", 
                           choice="mode_cat", varying=c(22:27))  

#average predicted probability for using each mode before and after intervention
apply(predict(mod.1, type = "response", newdata = dat.logit), 2, mean)
apply(predict(mod.1, type = "response", newdata = dat.1.logit), 2, mean)
