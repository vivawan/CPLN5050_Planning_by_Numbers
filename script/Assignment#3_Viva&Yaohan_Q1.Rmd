...
```{r reclassify 2012}
# plot the percentage of voting results
...
for (i in 1:length(var.12)){
  ...
  table.12[[i]] <- dat.12 %>%
    group_by(!!sym(var), candidate) %>%
    summarise(count = n(), .groups = 'drop') %>%
    group_by(!!sym(var))%>%
    mutate(percentage = round(count/sum(count) * 100, digits = 0),
           variable = var,
           category = as.character(!!sym(var)),
           vote = as.character(candidate))%>%
    ungroup()%>%
    select(variable, category, vote, count, percentage)
}
bind_rows(table.12) %>%
...
dat.12 %>%
  ...
  summarise(
    ...
    .groups = 'drop'  # Avoids retaining the grouping structure in the result
  ) %>%
...
```{r summary 16}
# table
var.16 <- colnames(dat.16%>%select(-year, -candidate))
table.16<- list()
