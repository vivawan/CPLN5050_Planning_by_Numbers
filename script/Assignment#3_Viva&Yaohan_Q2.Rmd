...
dat <- dat %>%
  mutate(age_cat_re = factor(case_when(
    ...
    age_cat %in% c('25-34', '35-44', '45-54', '55-64', '65-74') ~ '25-74',
    age_cat %in% c('75-85', '86+') ~ 'above 75')))
...
dat <- dat %>%
  mutate(bike_num_cat_re = factor(case_when(
    bike_num_cat %in% c('0') ~ 'no bikes',
    bike_num_cat %in% c('1', '2', '3', '4', '5', '6', '7', '8', '9', '12') ~ 'with bikes')))
...
stargazer(mod3.1, mod3.2, type = "text")
...
stargazer(mod4.1, mod4.3, type = "text")
...
stargazer(mod5.1, mod5.3, type = "text")
...
stargazer(mod6.1, mod6.2, type = "text")
```
...
## table
var_cat <- c('gender_cat', 'race_cat_re', 'edu_cat_re', 'lic_dum', 'park_sub_cat',
             'transit_sub_cat', 'toll_accnt_dum', 'car_share_dum', 'income_cat_re')
table_cat <- list()

for (i in 1:length(var_cat)){
  table_cat[[i]] <- dat %>%
    group_by(!!sym(var_cat[[i]]), mode_cat) %>%
    summarise(count = n()) %>%
    ungroup() %>%
    ...
    mutate(percentage = round(count/sum(count) * 100, digits = 0),
           ...
           choice = as.character(mode_cat)) %>%
    ungroup() %>%
    select(variable, category, choice, count, percentage)}

bind_rows(table_cat) %>%
  kable(caption = "<span style='font-weight: bold; color: black;'>Summary of Categorical Variables for Mode Choice</span>") %>%
...
wrap_plots(plot_cat, ncol = 3) + 
  ...
  plot_annotation(title = "Percentage of Mode Choice by Category",
