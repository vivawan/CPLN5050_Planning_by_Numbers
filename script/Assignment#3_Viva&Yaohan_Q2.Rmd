...
dat <- dat %>%
  mutate(bike_num_cat_re = factor(case_when(
    ...
    bike_num_cat %in% c('1', '2', '3', '4') ~ 'one to four bikes',
...
## 
mod.1 <- mlogit (mode_cat ~ cost + time | gender_cat + age_cat + age_cat_re + race_cat + race_cat_re + edu_cat + edu_cat_re + lic_dum
                 + park_sub_cat + transit_sub_cat + veh_num + veh_num_cat + veh_num_cat_re + bike_num + bike_num_cat + bike_num_cat_re
                 + toll_accnt_dum + car_share_dum + income_cat + income_cat_re, data = dat.logit)
summary(mod.1)
```
