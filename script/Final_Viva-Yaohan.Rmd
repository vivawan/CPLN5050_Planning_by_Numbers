...
name_17 <- unique(tran_rider_17$stop_name)
name_stop <- tran_stop$STATION
not_in_station <- as.data.frame(setdiff(name_17, name_stop))
name_19 <- unique(tran_rider_19$stop_name)
sum(name_19 %in% tran_stop$STATION)
##
...
bike_rider_17 <- bike_rider_17_start %>%
  ...
  mutate(Total_Docks = replace_na(Total_Docks,
                                  floor(mean(Total_Docks, na.rm = TRUE)))) %>%
...
rm(list = setdiff(ls(), c("tran_line", "tran_stop", "bike_rider_17", "bike_rider_19", "mbta_core")))
