...
## Transit Ridership
tran_rider <- tran_rider %>%
  mutate(time_period = paste0(day_type_name, sep = "_", time_period_name))
...
bike_rider_17_start <- bike_rider_17 %>%
  separate(starttime, into = date, time, sep = " ")
...
rm(list = setdiff(ls(), c("tran_line", "tran_stop", "tran_rider_17", "tran_rider_19",
                          "bike_rider_17", "bike_rider_19", "bus_stop", "mbta_core")))
...
sld_layers <- st_layers("/Users/Viva Wan/Documents/GitHub/Local/505PBN/SmartLocationDatabaseV3/SmartLocationDatabase.gdb")

sld <- st_read("/Users/Viva Wan/Documents/GitHub/Local/505PBN/SmartLocationDatabaseV3/SmartLocationDatabase.gdb")

# subset 
sld <- sld %>%
  filter(CBSA_Name == "Boston-Cambridge-Newton, MA-NH")%>%
  st_transform(crs = 'EPSG:2249')

sld <- sld[tran_stop_buffer,]
