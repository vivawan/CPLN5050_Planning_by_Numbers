...
```{r final_data}
rm(list = setdiff(ls(), c("tran_line", "tran_stop", "tran_rider_19_period",
                          "tran_rider_19_total", "bike_rider_19_period", "bike_rider_19_total",
                          "bus_stop", "mbta_core", "radius", "acs.2019.stop", "nn_function")))
...
final_19_total_acs <- final_19_total_bike %>%
  ...
  mutate(total_rider_bike = round(total_rider_bike/3, digits = 0),
...
final_19_period_acs <- final_19_period_bike %>%
  ...
  mutate(time_hour = case_when(
    ...
    time_period == 'saturday_OFF_PEAK'~ 24,
    time_period == 'sunday_OFF_PEAK'~ 24
    ))%>%
...
```{r ridership per hour by time period}
# plot
final_19_period_acs %>% 
  ...
  geom_col()+
  labs(title = "Mean Ridership Per Hour by Time Period",
       x = "Time Period",
       y = "Mean Ridership") +
  theme_minimal()
```
...
"log.total_rider_tran" "bus_stop"             "pop.den"             
"commuter.tot"         "white.share"       
"tran.share"           "veh.ownership"        "medInc"                   
"employ.rate"          "log.total_rider_bike" "log.bike.share"           "log.job.den"           
...
mod.2 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + pop.den + commuter.tot + log(job.den+1) + white.share + log(bike.share+1) + tran.share + veh.ownership + medInc, data = final_19_total_acs)
summary(mod.2)

mod.3 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + pop.den + commuter.tot + log(job.den+1) + white.share + log(bike.share+1) + tran.share + medInc, data = final_19_total_acs)
summary(mod.3)

mod.4 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + pop.den + commuter.tot + white.share + log(bike.share+1) + tran.share + medInc, data = final_19_total_acs)
summary(mod.4)
...
mod.6 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1) + bus_stop + pop.den + commuter.tot + white.share + log(bike.share+1), data = final_19_total_acs)
summary(mod.6)
```
...
```{r step3}
## 2019
mod.7 <- lm(log(hour_rider_tran + 1) ~ log(hour_rider_bike + 1) + time_period_cat + bus_stop + pop.den + commuter.tot + white.share + log(bike.share+1), data = final_19_period_acs)
summary(mod.7)
vif(mod.7)
```
...
```{r step4}
## 2019
mod.8 <- lm(log(hour_rider_tran + 1) ~ log(hour_rider_bike + 1)*time_period_cat + bus_stop + pop.den + commuter.tot + white.share + log(bike.share+1), data = final_19_period_acs)
summary(mod.8)
```
