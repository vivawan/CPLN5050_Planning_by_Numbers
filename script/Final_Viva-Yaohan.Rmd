...
tran_rider_17_period <- tran_rider %>%
  ...
  group_by(time_period, stop_name) %>%
  summarise(aver_ons = sum(average_ons),
            aver_offs = sum(average_flow)) %>%
  ungroup() %>%
  mutate(total_rider = aver_ons + aver_offs) %>%
  select(-aver_ons, -aver_offs) %>%
  left_join(., tran_stop, by = c("stop_name" = "STATION")) %>%
  na.omit() %>%
  select(-LINE, -ROUTE) %>%
  rename(terminus = TERMINUS) %>%
  relocate(time_period, .after = stop_name)

### Total Transit Ridership
tran_rider_17_total <- tran_rider_17_period %>%
  st_drop_geometry() %>%
  group_by(stop_name) %>%
  summarise(total_rider = sum(total_rider)) %>%
  left_join(., tran_stop, by = c("stop_name" = "STATION")) %>%
  na.omit() %>%
  select(-LINE, -ROUTE) %>%
  rename(terminus = TERMINUS)
