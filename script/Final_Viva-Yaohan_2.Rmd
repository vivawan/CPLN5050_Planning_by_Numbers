...
# get variables
acs_variable_list.2019 <- load_variables(201, "acs5")%>%
...
```{r final_data}
rm(list = setdiff(ls(), c("tran_line", "tran_stop", 
                          "tran_rider_19_period", "tran_rider_19_total", "bike_rider_19_period", "bike_rider_19_total",
...
```{r step1}
rm(list = setdiff(ls(), c("final_19_total_acs")))


## 2019
modb.0 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1), data = final_19_total_acs)
summary(modb.0)
```

```{r acs buffer}

modb.0 <- lm(total_rider_tran ~ total_rider_bike, data = final_19_total_acs)
summary(modb.0)
```
...
```{r step2}
hist(final_19_total_acs$)
table(final_19_total_acs$)
...
modb.1 <- lm(log(total_rider_tran) ~ log(total_rider_bike + 1)*time_period, data = final_19_period_acs)
summary(modb.1)
vif(modb.1)
