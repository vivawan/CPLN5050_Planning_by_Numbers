...
# birth -> 1911-1942, 1943-1974, after1975
dat.12 %>%
  select(candidate, birth) %>% na.omit() %>%
  group_by(candidate,birth) %>%
  summarize(count.vote = n()) %>%
  ungroup() %>%
  group_by(birth)%>%
  mutate(per = round(count.vote/sum(count.vote)*100), digit= 0)%>%
  ungroup()%>%
    ggplot(aes(birth,per)) +
      geom_bar(aes(fill= candidate), position="dodge", stat="identity") +
      scale_fill_viridis(discrete = TRUE) +
      labs(title = "Percentage of Voting by Birth Year in 2012") +
      theme_bw()
