...
tran_line2<- tran_line%>%
  filter(LINE != "SILVER")
st_write(obj = tran_line2,
         dsn = here::here("data/processed/tran_line.shp"))
...
final_19_period_acs <- final_19_period_bike %>%
  ...
  mutate(time_period_cat_2 = case_when(
    time_period %in% c("saturday_OFF_PEAK", "sunday_OFF_PEAK") ~ "weekend",
    time_period %in% c("weekday_VERY_EARLY_MORNING", "weekday_EARLY_AM", "weekday_AM_PEAK") ~ "weekday_morning",
    time_period %in% c("weekday_MIDDAY_BASE", "weekday_MIDDAY_SCHOOL", "weekday_PM_PEAK") ~ "weekday_midday",
    TRUE ~ "weekday_evening"))
```
...
```{r step3}
## 2019
mod.7 <- lm(log(hour_rider_tran + 1) ~ log(hour_rider_bike + 1) + time_period_cat + bus_stop + pop.den + commuter.tot + white.share + log(bike.share+1) + tran.share, data = final_19_period_acs)
summary(mod.7)
vif(mod.7)
```
...
```{r step4}
## 2019
mod.8 <- lm(log(hour_rider_tran + 1) ~ log(hour_rider_bike + 1)*time_period_cat + time_period_cat + bus_stop + pop.den + commuter.tot + white.share + log(bike.share+1) + tran.share , data = final_19_period_acs)
summary(mod.8)
plot(mod.8)
```
